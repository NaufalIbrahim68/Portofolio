              ...
              <nav className="nav hidden lg:block" aria-label="In-page jump links">
                <ul className="mt-16 w-max">
                  {navigationItems.map((item) => (
                    <li key={item.id}>
                      <a
                        className={`group flex items-center py-3 ${activeSection === item.id ? "text-slate-100" : "text-slate-400"
                          }`}
                        href={`#${item.id}`}
                        onClick={() => setActiveSection(item.id)}
                      >
                        <span
                          className={`nav-indicator mr-4 h-px transition-all group-hover:w-16 group-hover:bg-slate-100 ${activeSection === item.id ? "w-16 bg-slate-100" : "w-8 bg-slate-400"
                            }`}
                        ></span>
